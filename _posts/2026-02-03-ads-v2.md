...
layout: post
title: "≋👻 Apparition Delivery System v2.0 --- From Monolithic Scripts to Minimal Command Generation 👻≋"
date: 2026-02-03
author: Qweary
categories: [redteam, windows, ads, ntfs, apparition]
tags: [ redteam, ads, ntfs, windows, powershell, persistence, alternate data stream, llm, tradecraft  ]
...
-   🪄 NTFS Alternate Data Streams for hiding payloads

-   ⏰ Scheduled Task persistence

-   🔐 AES-256 encryption, with host-derived keys

-   🧪 Designed for CCDC competitions and real-world red team engagements
...
-   ❌ Not friendly to copy/paste or "one-shot" workflows
...
    scp /path/to/ADS-Dropper.ps1 target

         ↓

    powershell -ep bypass -f ADS-Dropper.ps1
...
│ ADS-Dropper.ps1            │   │ ADS-OneLiner.ps1         │

│ (Core logic)               │◄──│ (Command generator)      │

│ - Config generation        │   │ - Calls ADS-Dropper      │

│ - Randomization            │   │   in -GenerateOnly mode  │

│ - Encryption helpers       │   │ - Builds minimal cmds    │

│ - Stream name encoding     │   │ - Base64 encodes         │

│ Runs on: Linux OR Windows  │   │ Runs on: Linux only      │

└────────────────────────────┘   └──────────────────────────┘

                                           ↓

                                 Generates ~30-line one-liner

                                           ↓

                                 Paste into any PowerShell session
...
    $hp = Join-Path $env:ProgramData "SystemCache.dat"

} else {

    # Running on Linux (or WSL, GitHub Actions, Claude, etc.)
...
-   Debugging is now "run on Linux → copy → paste on Windows → cry"
...
Blue team just nuked your last beacon → social-engineer a user → paste 30-line one-liner → persistence back in <30 seconds.

Red team engagement

You have a 45-second window in an existing PS session → generate one-liner on attacker box → paste → clean exit → zero artifacts.
...
Stay curious, stay ethical, and keep those shells alive! 🐱‍💻
